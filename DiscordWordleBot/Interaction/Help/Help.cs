...
namespace DiscordWordleBot.Interaction.Help
{
    [Group("help", "取得說明")]
    public class Help : TopLevelModule<Service.HelpService>
    {
        ...
        public async Task H([Summary("指令名稱")] string command = "")
        {
            ...
            if (string.IsNullOrWhiteSpace(command))
            {
                ...
                embed.Title = "猜字小幫手 建置版本" + Program.VERSION;
#if DEBUG
                embed.Title += " (測試版)";
#endif
                embed.WithDescription(File.ReadAllText(DiscordWordleBot.Utility.GetDataFilePath("HelpDescription.txt")).Replace("\\n", "\n") + $"\n\n您可以透過: [Patreon]({PatreonUrl}) 或 [Paypal]({PaypalUrl}) 來贊助輔助小幫手");
                await RespondAsync(embed: embed.Build());
                return;
            }
